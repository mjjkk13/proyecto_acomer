...
const CursosDocente = () => {
  ...
  const handleAsistencia = (idEstudiante) => {
    setAsistencias((prev) => {
      const nuevoEstado = { ...prev, [idEstudiante]: !prev[idEstudiante] };
              ...
              <div className="space-y-2 flex-grow overflow-auto">
                ...
                {totalPaginas > 1 && (
                  <div className="flex justify-center mt-4 space-x-4">
                    <button className="btn btn-sm" onClick={() => cambiarPagina(paginaActual - 1)} disabled={paginaActual === 1}>
                      Anterior
                    </button>
                    <span className="flex items-center px-3">Página {paginaActual} de {totalPaginas}</span>
                    <button className="btn btn-sm" onClick={() => cambiarPagina(paginaActual + 1)} disabled={paginaActual === totalPaginas}>
                      Siguiente
                    </button>
                  </div>
                )}
